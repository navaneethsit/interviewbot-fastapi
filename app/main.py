...
# Save to DB
async def save_resume_to_db(filename: str, content: str):
...
async def resume_upload(file: UploadFile = File(...), background_tasks: BackgroundTasks = None):
    try:
        content = await extract_resume_text(file)
        background_tasks.add_task(save_resume_to_db, file.filename, content)
        return {"message": "Resume uploaded and stored"}
